...
export const acceptedBountiesProcedure = publicProcedure.query(async () => {
  try {
    ...
    const q = query(
      bountiesRef,
      where('acceptedHunters', 'array-contains', currentUser.uid)
    );
    ...
    console.log('Accepted bounties:', bounties.length);
    return bounties;
  } catch (error: any) {
    console.error('Failed to get accepted bounties:', error);
    throw new Error(error.message || 'Failed to get accepted bounties');
  }
});
